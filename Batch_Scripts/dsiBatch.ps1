...
$driveAndPathToParticipants = $args[0];
$subjectId = $args[1];
$pathToDsiStudio = $args[2];
$numberOfTracts = $args[3];
...
Invoke-Expression "$pathToDsiStudio/dsi_studio.exe --action=rec  --thread_count=10 --source=$driveAndPathToParticipants/sub-$subjectId/dsi-data/data.src.gz --output=$driveAndPathToParticipants/sub-$subjectId/dsi-data/data.src.gz.gqi.1.25.fib.gz --method=4 --param0=1.25 --scheme_balance=1 --check_btable=1"
...
Start-Process $pathToDsiStudio/dsi_studio.exe -ArgumentList "--action=rec  --thread_count=10 --source=$driveAndPathToParticipants/sub-$subjectId/dsi-data/data.src.gz --output=$driveAndPathToParticipants/sub-$subjectId/dsi-data/data.src.gz.gqi.1.25.fib.gz --method=4 --param0=1.25 --scheme_balance=1 --check_btable=1" -wait -UseNewEnvironment -RedirectStandardOutput "$driveAndPathToParticipants/sub-$subjectId/logs/dsi-rec-output.log" -RedirectStandardError "$driveAndPathToParticipants/sub-$subjectId/logs/dsi-rec-error.log"


Write-host "Tracking fibres for diffusion data of subject: $subjectId..."
Start-Process $pathToDsiStudio/dsi_studio.exe -ArgumentList "--action=trk --source=$driveAndPathToParticipants/sub-$subjectId/dsi-data/data.src.gz.gqi.1.25.fib.gz --fiber_count=$numberOfTracts --initial_dir=0 --thread_count=10 --fa_threshold=0.02908 --step_size=0.625 --turning_angle=60 --smoothing=0 --min_length=10 --max_length=300 --method=0 --ref=$driveAndPathToParticipants/sub-$subjectId/data/bert/mri/aparc+aseg.nii --output=$driveAndPathToParticipants/sub-$subjectId/dsi-data/1m0.trk" -wait -UseNewEnvironment -RedirectStandardOutput "$driveAndPathToParticipants/sub-$subjectId/logs/dsi-trk-0-output.log" -RedirectStandardError "$driveAndPathToParticipants/sub-$subjectId/logs/dsi-trk-0-error.log"