...
def getPipelineSuccessStatus() -> "Dict[str, Tuple[Optional[bool], str]]":
    ...
    if not config.PIPELINE_SUCCESS_FILE.is_file():
        ...
        existing_success: pd.DataFrame = pd.read_csv(
            config.PIPELINE_SUCCESS_FILE)
        # Convert existing DataFrame to a dictionary for easier updating
        existing_success_dict = dict(
            zip(
                existing_success['Step'],
                zip(existing_success['Success'],
                    existing_success['Last Modified']),
            )
        )
...
def getPipelineSuccessStatusForAllSubjects() -> "Dict[str, Dict[str, Tuple[Optional[bool], str]]]":
    ...
    for subjectId in config.ALL_SUBJECTS:
        ...
        if not file_path.exists():
            ...
            subject_success_dict = {stepFn.__name__: (
                None, "") for stepFn in g.allSteps.keys()}
        else:
...
def updateBatchStatus() -> bool:
    ...
    df: pd.DataFrame = pd.DataFrame.from_dict(
        flattened_data, orient='index').reset_index()
