...
def createLabelToRandomWordLookup(x: "Optional[pd.Series[int]]" = None, y: "Optional[pd.Series[int]]" = None) -> dict[int, str]:
...
def convertNumericalModuleToWords(xory: "pd.Series[int]", label_to_word_map_xory: "dict[int,str]") -> "pd.Series[str]":
...
def convertNumericalModulesToWords(x: "pd.Series[int]", y: "pd.Series[int]", dataIsMapped: bool = False) -> "tuple[pd.Series[str], pd.Series[str]]":
    if(dataIsMapped):
...
def calcModuleSizes(x_module: "Union[pd.Series[str], pd.Series[int]]", y_module: "Union[pd.Series[str], pd.Series[int]]", xy_surface_area: "npt.NDArray[np.float64]") -> "tuple[np.float64, np.float64, np.float64]":
...
def mapAllModulesToSameSet(x: "Union[pd.Series[int],pd.Series[str]]", y: "Union[pd.Series[int],pd.Series[str]]", mappedNames: "Optional[dict[str,str]]" = None) -> "tuple[dict[str,str],Union[pd.Series[int],pd.Series[str]], Union[pd.Series[int],pd.Series[str]]]":
    row_ind: list[int]
    col_ind: list[int]
    ...
    if (mappedNames is None):
        ...
        cont_matrix: npt.NDArray = cast(npt.NDArray, contingency_matrix(
            labels_true=x.to_numpy(),
            labels_pred=y.to_numpy(),
            sparse=False))
        ...
        g.logger.info("Computing normalised levenshtein distance for all possible module pairs")
        ...
        row_ind, col_ind = linear_sum_assignment(
            (0.5 * (1 - norm_cont_matrix)) + (0.5 * norm_lev_matrix))  # type:ignore
...
def calculateLevenshteinDistance(x: "Union[pd.Series[str],pd.Series[int]]", y: "Union[pd.Series[str],pd.Series[int]]") -> int:
...
def calculateNormalisedLevenshteinDistance(x: "Union[pd.Series[str],pd.Series[int]]", y: "Union[pd.Series[str],pd.Series[int]]") -> float:
    # This is a token based approach to calculate the distance between modules.
    distance: float = Levenshtein.distance(x.to_list(), y.to_list(
    )) / max(x.size, y.size)
    return distance
