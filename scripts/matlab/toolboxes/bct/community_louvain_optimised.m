function [M,Q]=community_louvain_optimised(W,gamma,M0,B)
...
disp('Starting community detection...');
...
disp('Initializing parameters and variables...');
...
if strcmp(type_B,'negative_sym') || strcmp(type_B,'negative_asym')
    disp('Processing negative weights...');
...
disp(['Objective function type: ', type_B]);
...
% Use parfor to parallelize this loop over modules
disp('Calculating initial node-to-module degree...');
parfor m=1:max(Mb)
    Hnm(:,m)=sum(B(:,Mb==m),2);
end
...
while Q-Q0>1e-10
    ...
    disp('Updating module assignments...');
    ...
    disp('Pooling weights of nodes...');
    n=max(Mb);                                          % new number of modules
    B1_local=zeros(n);                                  % new weighted matrix for each worker

    % Use parfor to parallelize this loop
    parfor u=1:n
        % Initialize a temporary matrix for storing results of this iteration
        B1_temp = zeros(n, n);
        for v=u:n
        ...
        % Accumulate results to the local B1 matrix
        B1_local = B1_local + B1_temp;
    end
    B1 = B1_local;                                      % Assign the accumulated result to B1
    B=B1;
    ...
    disp(['Current modularity Q: ', num2str(Q)]);
end

disp('Community detection complete.');
disp(['Final modularity: ', num2str(Q)]);
