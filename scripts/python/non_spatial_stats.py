...
def runTests(subjectId: str, pathToXCsv: Path, pathToYCsv: Path, pathToOutputtedXlsx: Path) -> bool:
    ...
    try:
        ...
        df_x_truth_with_range, df_y_truth_with_range = convertResultsToDataFrames(results_x_truth_with_range, results_y_truth_with_range)

        # Write results to Excel
        with pd.ExcelWriter(path=pathToOutputtedXlsx, engine='xlsxwriter') as writer1:
            df_x_truth_with_range.to_excel(writer1, sheet_name='X as Truth', index=False)
            df_y_truth_with_range.to_excel(writer1, sheet_name='Y as Truth', index=False)
            writer1.close()

        append_df_to_csv(df=df_x_truth_with_range, filename=config.STAT_FILE)
        append_df_to_csv(df=df_y_truth_with_range, filename=config.STAT_FILE)
