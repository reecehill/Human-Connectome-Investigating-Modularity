...
close all;
F = sort(padarray(allBrainData.leftHemisphere.surf.faces(:,4), 6,-2,'post'));
F(F==0)=-1;
reshapedF = reshape(F, 162, 162);
figure;
heatmap(reshapedF, 'ColorScaling','log', 'ColorMap',lines);
figure;
F = sort(padarray(allBrainData.leftHemisphere.surf.faces(:,5), 6,-2,'post'));
F(F==0)=-1;
reshapedF = reshape(F, 162, 162);
heatmap(reshapedF, 'ColorScaling','log', 'ColorMap',lines);
