...
def mask_removeMissing(
    x: "pd.Series[int]", y: "pd.Series[int]"
...
def ffill(
    ...
    x_filled: "pd.Series[int]" = x.replace(-1, np.nan).ffill(limit=1).bfill(limit=1)
    y_filled: "pd.Series[int]" = (
...
def white_noise(
    ...
    **kwargs: "Union[pd.Series[int], pd.Series[str]]"
...
def filter_by_parent(
    ...
    for y_module_name in y_maskedby_y.unique():
        ...
        x_module_name: int = x_maskedby_y[y_maskedby_y == y_module_name].mode()[0]
