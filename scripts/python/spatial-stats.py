...
def generate_interpretation(test_name, score):
...
for test_name, test_func in test_functions_with_range:
    try:
        ...
        score_x_imported_random_y = test_func(x_clean, np.random.choice(np.unique(y_clean), size=len(y_clean)))
        score_x_random_y_imported = test_func(np.random.choice(np.unique(x_clean), size=len(x_clean)), y_clean)
...
for test_name, test_func in test_functions_with_range:
    try:
        ...
        score_y_imported_random_x = test_func(y_clean, np.random.choice(np.unique(x_clean), size=len(x_clean)))
        score_y_random_x_imported = test_func(np.random.choice(np.unique(y_clean), size=len(y_clean)), x_clean)
