        ...
        with TextIOWrapper(buffer=process.stdout):
            for line in iter(process.stdout.readline, b""):  # b'\n'-separated lines
                ...
                if re.search("[a-zA-Z]", line):
                    ...
                    g.logger.debug(
                        msg="Output omitted (does not contain alpha characters)"
                    )

        if process.returncode != 0:
            if process.returncode is None:
                g.logger.warning(
                    f"{cmdLabel}: Command did not return a code. We will assume it was successful, but check logs."
                )
                return True
            else:
                g.logger.error(
                    f"{cmdLabel}: Command failed with exit code {process.returncode}"
                )
