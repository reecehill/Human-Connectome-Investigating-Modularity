...
def findModularity(subjectId: str) -> bool:
    ...
    L_optimalModules: 'Union[list[list[np.int8]],NodeClustering,npt.NDArray[np.int8]]' = [
    ]
    R_optimalModules: 'Union[list[list[np.int8]],NodeClustering,npt.NDArray[np.int8]]' = [
    ]
    ...
    if config.NETWORKX_ALGORITHM == 'leiden_communities':
        ...
        L_optimalModules = algorithms.leiden(
            g_original=L_graph, weights='weight').communities  # type: ignore
        R_optimalModules = algorithms.leiden(
            g_original=R_graph, weights='weight').communities  # type: ignore
    elif config.NETWORKX_ALGORITHM == 'bayan':
        L_optimalModules = algorithms.bayan(
            g_original=L_graph, time_allowed=1, resolution=1)  # type: ignore
        R_optimalModules = algorithms.bayan(
            g_original=R_graph,  time_allowed=1, resolution=1).communities  # type: ignore
    elif config.NETWORKX_ALGORITHM == 'frc_fgsn':
        L_optimalModules = algorithms.frc_fgsn(
            g_original=L_graph, theta=1, eps=0.5, r=3000).communities  # type: ignore
        R_optimalModules = algorithms.frc_fgsn(
            g_original=R_graph, theta=1, eps=0.5, r=3000).communities  # type: ignore
    elif config.NETWORKX_ALGORITHM == 'principled_clustering':
        L_optimalModules = algorithms.principled_clustering(
            g_original=L_graph, cluster_count=5).communities  # type: ignore
        R_optimalModules = algorithms.principled_clustering(
            g_original=R_graph, cluster_count=5).communities  # type: ignore
    elif config.NETWORKX_ALGORITHM == 'scan':
        L_optimalModules = algorithms.scan(
            g_original=L_graph, epsilon=0.2, mu=10).communities  # type: ignore
        R_optimalModules = algorithms.scan(
            g_original=R_graph, epsilon=0.2, mu=10).communities  # type: ignore
        ...
        try:
            ...
            L_optimalModules = groupModulesByLabel(np.array(L_optimalModules_list, dtype=np.int8).transpose())
            R_optimalModules_list = sklearn.cluster.spectral_clustering(
                affinity=R_adj, n_clusters=5, assign_labels="discretize")
            R_optimalModules = groupModulesByLabel(np.array(R_optimalModules_list, dtype=np.int8))

        except Exception as e:
            g.logger.error(
                f"Failed to run spectral clustering algorithm: {str(e)}")
        ...
        L_optimalModules = [list(community) for community in nx_community.greedy_modularity_communities(
            L_graph, weight='weight', resolution=L_optimalGamma, cutoff=10, best_n=10)]  # type: ignore
        R_optimalModules = [list(community) for community in nx_community.greedy_modularity_communities(
            R_graph, weight='weight', resolution=R_optimalGamma, cutoff=10, best_n=10)]  # type: ignore
        ...
        L_optimalModules_gen = nx_community.fast_label_propagation_communities(
            L_graph, weight='weight')  # type: ignore
        L_optimalModules = np.array([list(community) for community in deque(
            L_optimalModules_gen, maxlen=len(L_graph))])
        R_optimalModules_gen = nx_community.fast_label_propagation_communities(
            R_graph, weight='weight')  # type: ignore
        R_optimalModules = np.array([list(community) for community in deque(
            R_optimalModules_gen, maxlen=len(R_graph))])
    elif config.NETWORKX_ALGORITHM == 'async_fluid':
        L_optimalModules = algorithms.async_fluid(
            L_graph, k=config.NETWORKX_FLUID_K)
        R_optimalModules = algorithms.async_fluid(
            R_graph, k=config.NETWORKX_FLUID_K)
    elif config.NETWORKX_ALGORITHM == 'girvan_newman':
        L_optimalModules_gen = nx_community.girvan_newman(
            L_graph)  # type: ignore
        L_optimalModules = np.array([list(community) for community in deque(
            L_optimalModules_gen, maxlen=len(L_graph))])
        R_optimalModules_gen = nx_community.girvan_newman(
            R_graph)  # type: ignore
        R_optimalModules = np.array([list(community) for community in deque(
            R_optimalModules_gen, maxlen=len(R_graph))])
    elif config.NETWORKX_ALGORITHM == 'k_clique_communities':
        L_optimalModules_gen = nx_community.k_clique_communities(
            L_graph, k=3, cliques=nx.find_cliques(L_graph))  # type: ignore
        L_optimalModules = np.array([list(community) for community in deque(
            L_optimalModules_gen, maxlen=len(L_graph))])
        R_optimalModules_gen = nx_community.k_clique_communities(
            R_graph, k=3, cliques=nx.find_cliques(R_graph))  # type: ignore
        R_optimalModules = np.array([list(community) for community in deque(
            R_optimalModules_gen, maxlen=len(R_graph))])
    else:
        g.logger.info(
...
def findOptimalGamma(subjectId: str, weighted_matrix: str) -> np.float64:
    ...
    all_gammas = cast('npt.NDArray[np.float64]', np.arange(
        config.NETWORKX_GAMMA_START, config.NETWORKX_GAMMA_END, config.NETWORKX_GAMMA_STEP))
...
# Get computed adjacency matrix
    ...
    numel_adj_matrix: np.int32 = computed_matrix.shape[0] * \
        computed_matrix.shape[1]
    density_of_random_matrix = float(
        number_of_edges_to_create / numel_adj_matrix)
    ...
    with Pool() as pool:
        for gamma in all_gammas:
            ...
            Q_corts = pool.map(process_iteration, [(
                gamma, iteration_index, G_corts, 'cortical') for iteration_index in all_iterations])
            Q_rands = pool.map(process_iteration, [(
                gamma, iteration_index, G_rand, 'random') for iteration_index in all_iterations])
...
def runLouvainAlgorithm(graph: Graph, gamma: np.float64) -> 'tuple[npt.NDArray[np.int8] | NodeClustering, np.float64]':
    if config.NETWORKX_ALGORITHM == 'louvain_communities':
        communities: 'list[set[int]]' = nx_community.louvain_communities(
            graph, weight='weight', resolution=gamma)  # type: ignore
    elif config.NETWORKX_ALGORITHM == 'greedy_modularity_communities':
