...
def perModuleStat(x_final: "pd.Series[str]", y_final: "pd.Series[str]", x: "pd.Series[int]", y: "pd.Series[int]", xy_surface_area: "pd.DataFrame") -> None:
    ...
    for (_, y_module_name) in enumerate(y_module_names):
        ...
        if ("padXModules" == "padXModules" and 1 == 1):
            x_final_module_all_paddedIndexes: "list[int]" = pad_indexes(
        ...
        if ("randomiseMissingYModules" == "randomiseMissingYModules" and 1 == 1):
            ...
            y_final_module_within_x_with_missing: "pd.Series[int]" = y[x_final_module_all.index].replace([
                                                                                                         0, 1, "1"], np.nan)
            y_final_module_within_x_with_missing[y_final_module_within_x_with_missing.isna(
            )] = np.random.normal(loc=0, scale=1, size=y_final_module_within_x_with_missing.isna().sum())
        else:
            y_final_module_within_x_with_missing: "pd.Series[int]" = y[x_final_module_all.index].replace([
                                                                                                         0, 1, "1", np.nan], -1)
