...
# From root, resolvable by Path.resolve(). Enter here to override automatic finding.
wbCommandPath: str = "/gpfs01/software/imaging/workbench/workbench-1.5.0/bin_rh_linux64/wb_command"
...
WB_COMMAND = getPathOfExecutable(
    executable="wb_command", userSubmitted=wbCommandPath)
# COMPOSITE PATHS DEPENDANT ON PREV PATHS
FMRI_SCALAR_PATH_CORTICAL: str = str(Path(IMAGES["FMRI"]["LOW_RES"]["DATA"]["FOLDER"]) /
                                     IMAGES["FMRI"]["LOW_RES"]["DATA"]["PATH"]).replace(".dscalar.nii", "_ROI.dscalar.nii")
...
def setCurrentBatch(currentBatch: str) -> None:
    ...
    BATCH_SUCCESS_FILE = DATA_DIR / \
        f'batchSuccess-{CURRENT_BATCH}-{TIMESTAMP_OF_SCRIPT}.csv'
...
def setSubjectStepSuccess(subjectStepSuccess: Optional[bool], reset: bool = False) -> None:
    ...
    if (SUBJECT_STEP_SUCCESS == None):
        ...
        raise ValueError("Invalid value for SUBJECT_STEP_SUCCESS. Expected bool, got {}.".format(
