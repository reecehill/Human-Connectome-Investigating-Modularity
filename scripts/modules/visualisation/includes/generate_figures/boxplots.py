...
def generate_dynamic_boxplot(
    filtered_df: pd.DataFrame,
    statistic: List[str],
    applied_filters: Dict[str, Any],
    group_by: List[str],  # Dynamic list of grouping columns
    title_append: str = "",
    output_path: Union[str, Path] = "boxplot_figure.png",
):
    """
    Generates a boxplot grouped by a dynamic list of columns.
    ...
    ax = sns.boxplot(data=melted_df, x=x_label, y=y_label, hue="Score Type")
    ...
    if len(group_by) > 1:
        ...
        # Add zebra pattern (greyscale) to boxplot
    ...
    # Customize the plot
    title = f"Boxplot of Scores Grouped by {', '.join(group_by)}"
    ...
    plt.close()
    print(f"Boxplot figure saved as '{output_path}'.")
