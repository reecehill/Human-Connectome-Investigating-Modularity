...
def processDiffusionTracts(subjectId: str) -> bool:
    ...
    subjectHiResSurfaceFolder = (
        config.SUBJECT_DIR / config.IMAGES["FMRI"]["HIGH_RES"]["SURFACE"]["FOLDER"]
    )
    subjectHiResSurface_left = subjectHiResSurfaceFolder / config.IMAGES["FMRI"][
        "HIGH_RES"
    ]["SURFACE"]["L_HEMISPHERE_PATH"].replace("$subjectId$", subjectId)
    subjectHiResSurface_right = subjectHiResSurfaceFolder / config.IMAGES["FMRI"][
        "HIGH_RES"
    ]["SURFACE"]["R_HEMISPHERE_PATH"].replace("$subjectId$", subjectId)
    ...
    localFilesToExist: "list[Path]" = [
        ...
        for label in anatomicalLabelsToExist
    ]
    ...
    if config.USE_PRESET_DOWNSAMPLED_MESH:
        ...
        createDirectories(
            directoryPaths=[subjectDownsampledFolder],
            createParents=True,
