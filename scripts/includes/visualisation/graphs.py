...
def plotRoiRegion() -> None:
  ...
  faces = np.hstack([[3, *face]
                    for face in xy_faces.to_numpy()]).astype(int)

  mesh = pv.PolyData(xy_vertices.to_numpy(), faces)
  plotter = pv.Plotter()
  plotter.add_mesh(mesh, color="cyan", show_edges=True,
                  edge_color="black", opacity=0.8, smooth_shading=True)
  light = pv.Light(position=(5, 5, 5),
                  focal_point=(0, 0, 0), color='white')
